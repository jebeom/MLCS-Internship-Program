...
"""
data = yf.download(ticker, start="2020-01-01", end="2024-05-31")
...
for epoch in range(epochs):
    ...
    print(f'Epoch {epoch+1}/{epochs}, Loss: {train_loss/len(train_loader):.7f}')
...
with torch.no_grad():
    ...
    for sequences, target in test_loader:
        input_seq = sequences.to(device)
        pred_seq = []
        for _ in range(pred_length):
            output = model(input_seq)
            pred_seq.append(output.item())
